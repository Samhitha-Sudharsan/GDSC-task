...
class music_cog(commands.Cog):
    ...
    def play_next(self) :   
        if len(self.music_queue) > 0:
            ...
            self.vc.play(discord.FFmpegPCMAudio(m_url,**self.FFMPEG_OPTIONS),after = lambda e: self.play_next()) #recurssive
        else :
            self.is_playing = False
    ...
    async def play(self,ctx,*args):
        ...
        if voice_channel is None:
            ...
            if type(song) == type(True):
                await ctx.send("Could not get the song. Try a different word")
            else :
